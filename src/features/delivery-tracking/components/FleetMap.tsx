...
// หมุด messenger — วงกลมเขียวเมื่อ GPS สด, เทาเมื่อสัญญาณเก่า
const messengerIcon = (fresh: boolean) =>
  L.divIcon({
    className: '',
    iconSize: [22, 22],
    iconAnchor: [11, 11],
...
export function FleetMap({ focusOrder }: FleetMapProps) {
  ...
  }, [activeOrderStops, focusOrder, messengers, selectedId]);
  ...
  const allPoints = useMemo<[number, number][]>(
    ...
    [currentDestinations, messengers],
  );
        ...
        >
          ...
          {messengers.map((messenger) => {
            ...
            return (
              <Marker
                key={messenger.id}
                icon={messengerIcon(isFresh(messenger.latest?.recordedAt))}
                position={point}
